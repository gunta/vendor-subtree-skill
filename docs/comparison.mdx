...
> How should you feed library and dependency knowledge to AI coding agents?

This document compares every major approach — from native git mechanisms to dedicated tools and services — for providing repository context to coding agents like Claude Code, Cursor, Copilot, and others.
...
```mermaid
flowchart TB
  Q["AI coding agent needs<br/>library context"]
...
**Two fundamental strategies, with one hybrid layer**:
...
Opinionated, multi-strategy CLI that wraps git subtree, git submodules, local ignored clones, and shared cache-linked checkouts. Distributed both as a standalone npm package and as a Claude Code skill. Built on Effect-TS.

```bash
bunx ingraft@latest                          # scan package.json, ask what to vendor
bunx ingraft@latest init                     # set up vendoring
...
flowchart TB
  ...
  Skill --> Router{Intent}
  Router -->|"vendor X"| Add["ingraft add"]
...
**Best for**: Teams that want git subtree's benefits without rough edges — especially in monorepos or when vendoring npm packages with version-sync.
...
**Best for**: Fork-backed editable vendors, where the team needs to patch the external repo and possibly send those commits upstream. **Not recommended as the default read-only agent context path** because initialization failures are a reliability risk. Use subtree for read-only source; use a fork-backed submodule when ownership of vendor patches matters more than zero-step checkout.
...
Three projects wrap `git subtree` for agent vendoring. All inspired by the Effect post; all created within weeks of each other in 2026.
...
| Want the agent to invoke the right vendoring command from natural language | **ingraft** + Claude Code               |
...
Submodules become the better choice when the vendor itself is meant to be edited.
In that case, point the submodule at a fork branch so the vendor patch history
lives in the fork and the parent repository only records the gitlink pointer.
...
1. **For serious agent-driven development, vendored source is the gold standard.** The Effect team's argument — agents read code better than docs — is architecturally sound. Whether you use raw `git subtree`, **ingraft**, or **Vendir**, the underlying win is the same: navigable source in the workspace.

2. **The wrapper choice matters less than the wrapping choice.** Both ingraft and Vendir are inspired by the Effect post. Pick ingraft for multi-strategy + manifest scanning + monorepo support + skill integration. Pick Vendir for a minimal subtree-only wrapper with TOML config. Both beat running raw commands.
...
5. **Submodules are not the default, but they have one strong job.** For read-only agent context, the initialization step is a reliability killer, so use subtree. For durable vendor patches, use a fork-backed submodule so changes live in the fork and can be upstreamed cleanly.
...
7. **The landscape is moving fast.** ingraft, Vendir, mgrep, BTCA-3, and OpenSrc are all young or recently rebuilt. Expect more tooling. The fundamental insight — **agents work best with navigable source code in their workspace** — is unlikely to change.
